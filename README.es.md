...
El script **solo soporta el containerd image store** (Docker >= 25 con `"features": { "containerd-snapshotter": true }` en daemon.json). El image store clásico no puede adjuntar credenciales a un mirror con token — Docker no adjunta credenciales a las pulls de `registry-mirrors` ([moby#30880](https://github.com/moby/moby/issues/30880)) — y cuando aún no está habilitado el script **ofrece añadirlo a `/etc/docker/daemon.json`** (responde `y` o `yes`; el almacén cambia en el siguiente reinicio de Docker):
