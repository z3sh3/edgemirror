...
**拉取报 `network is unreachable`？** 说明镜像域名解析到 IPv6（AAAA），但本机没有 IPv6 路由，containerd 会先拨 AAAA 地址而失败。任选其一修复：给主机关闭 IPv6（`sysctl -w net.ipv6.conf.all.disable_ipv6=1`，写进 `/etc/sysctl.d/99-disable-ipv6.conf` 并重启 Docker）、在 `/etc/hosts` 固定该域名的 IPv4、或在 CDN/DNS 侧关闭该域名的 IPv6。
